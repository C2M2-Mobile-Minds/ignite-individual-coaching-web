...
- **Issue #5 — Field validation**: `js/formEngine.js` blocks "next" while a visible `required` field is empty. Pure helpers `isFieldFilled(field, answers)` (empty-check per type: trimmed string / defined radio / `true` bare checkbox / non-empty array for a checkbox group) and `validateStep(step, answers)` (invalid required ids over `visibleFields`, so hidden conditionals like `como_chegou_outro` are skipped). `state.errors` is a `Set` of field ids — populated on a blocked `goNext`, cleared per field by the answer setters as the user edits (text: error node removed in place, no re-render; radio/checkbox: re-render omits it), and reset wholesale on `goNext`/`goBack`. `renderStep` appends `<span class="error" data-for="<id>" role="alert">` with `t("validation.required")` and sets `aria-invalid` on the control. Error copy: `validation.required` key added to `locales/pt-PT.json` (full string pass is issue #6). Tests extended in `test/formEngine.test.mjs`.
