...
| `email` | email | yes | — | |
| `como_chegou` | checkbox (multiple) | yes | `instagram`, `recomendacao`, `pesquisa`, `outro` | how they found Ignite |
| `como_chegou_outro` | text | yes | — | shown only when `como_chegou` includes `outro` |
| `objetivo_treino` | checkbox (multiple) | yes | `perda_peso`, `ganho_massa`, `condicao_fisica`, `saude_bem_estar`, `gestacao_posparto` | training goals; `gestacao_posparto` drives the branch |
