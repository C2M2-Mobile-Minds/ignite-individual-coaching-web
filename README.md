...
│   ├── theme.js           # accent palette per objetivo_treino + applyTheme (CSS custom props)
...
The page's accent colour follows the training objective picked on Página 1 (`objetivo_treino`): `js/theme.js` holds one config object mapping each option id to an accent palette and, on every step render, writes it onto `:root` as CSS custom properties (`--accent`, `--accent-bright`, `--accent-hover`, `--accent-soft`). Multiple objectives selected → the first in schema order wins (interim rule, pending client confirmation); none selected → the default green. Palettes are placeholders until the client delivers finals — only `theme.js` changes then.
...
flowchart TD
    ...
    C --> C2["Página 2b: Compromisso<br/><i>Confiança/compromisso a<br/>investir no acompanhamento</i>"]
...
| 2a. Sobre o treino | Onde treinas (casa/ginásio); qual a maior dificuldade neste momento (texto livre); como é a tua rotina de treinos (2-3x, 4-5x, 5+/semana); segues orientação alimentar de um nutricionista (sim/não) |
| 2b. Compromisso (ramo geral) | Confiança/compromisso a investir no acompanhamento on-line (sim/não) — sozinho na própria página (issue #45) |
